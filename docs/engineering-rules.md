...
- **R3.3 MUST** — a displayed or persisted security/status value is the server-derived EFFECTIVE truth, never raw config or a client-claimed value; a disagreeing client value is a spoof to reject + audit. *Why:* memory: honesty-in-surfaces. *Enforcement:* `[test]` + `[review]`. **Gate.**
- **R3.4 MUST** `[codex-lane]` — a readiness signal derives from the authoritative state machine, never from a proxy probe. Native-agent delivery readiness reads the broker FSM (`idle` = ready) via `channelsDeliveryStrategy.ts` / `agentSurfaceBroker.ts`; it does NOT sniff terminal/tmux panes. *Why:* an idle native agent was reported "blocked, message not delivered" because readiness was misclassified and a tmux submit-verify replayed — two real bugs found in manual E2E and fixed against the FSM. *Enforcement:* `[test]` (readiness classification per FSM state). **Gate.**
