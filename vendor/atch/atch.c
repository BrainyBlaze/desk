...
/* Returns the directory where session sockets are stored. BrainyBlaze fork
** always uses /tmp: tmpfs-backed, container-local, no cross-restart bleed. */
...
int no_ansiterm = 0;
/* Optional path to redirect the child's stderr (fd 2) before exec.
** When set via `-2 <path>`, the master opens this in append mode and
** dup2's it onto fd 2 in the child after forkpty (overriding the pty
** slave dup2). Lets the child write a sidechannel that does not mix
** with the TUI. */
const char *child_stderr_path = NULL;

/* Optional path to sanitize_env.so. When set via `-S <path>`, the master
** setenv()s LD_PRELOAD=<path> and the hidden GUARD var in the forked child
** right before execve. Keeps both values OUT of atch master's argv/env so
** /proc/<atch>/cmdline never exposes the GUARD secret. */
const char *child_se_preload = NULL;
	...
	while (*argc >= 1 && ***argv == '-') {
		...
		for (p = (*argv)[0] + 1; *p; ++p) {
			if (*p == 'E')
				...
				--(*argc);
				if (*argc < 1) {
					printf("%s: No stderr path "
					       "specified.\n", progname);
					printf("Try '%s --help' for more "
					       "information.\n", progname);
					return 1;
				}
				child_stderr_path = (*argv)[0];
				break;
			} else if (*p == 'S') {
				++(*argv);
				--(*argc);
				if (*argc < 1) {
					printf("%s: No sanitize_env.so path "
					       "specified.\n", progname);
					printf("Try '%s --help' for more "
					       "information.\n", progname);
					return 1;
				}
				child_se_preload = (*argv)[0];
				break;
			} else if (*p == 'T') {
				++(*argv);
				--(*argc);
				if (*argc < 1) {
					printf("%s: No event sink path "
					       "specified.\n", progname);
					printf("Try '%s --help' for more "
					       "information.\n", progname);
					return 1;
				}
				tstate_events_path = (*argv)[0];
				break;
			} else {
...
static void expand_sockname(void)
{
	...
	if (mkdir(dir, 0700) < 0 && errno != EEXIST)
		return;
	{
		int root = atch_storage_open_root(dir);
		if (root < 0)
			return;
		close(root);
	}
...
static int cmd_clear(int argc, char **argv)
{
	...
	fd = atch_storage_open_path_file(log_path, O_WRONLY | O_TRUNC, 0600);
	if (fd >= 0) {
		close(fd);
		if (!quiet)
...
static int cmd_tail(int argc, char **argv)
{
	...
	snprintf(log_path, sizeof(log_path), "%s.log", sockname);
	fd = atch_storage_open_path_file(log_path, O_RDONLY, 0600);
	if (fd < 0) {
		if (errno == ENOENT)
...
static void usage(void)
{
	printf(PACKAGE_NAME " - version %s, compiled on %s at %s.\n"
	       ...
	       "  -2 <path>\tRedirect child stderr (fd 2) to <path> "
	       "instead of pty\n"
