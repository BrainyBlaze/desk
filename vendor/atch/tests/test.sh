...
# BrainyBlaze fork pins the session dir to /tmp/.<basename>-<uid>/ regardless
# of HOME (see atch.c get_session_dir). HOME is still set above so anything
# else relying on it stays isolated; SESSDIR is what atch actually uses.
ATCH_BASENAME=$(basename "$ATCH")
SESSDIR="/tmp/.${ATCH_BASENAME}-$(id -u)"
# Scrub any state from earlier runs of this UID — without this, the second
# `sh tests/test.sh` invocation in the same container would see leftover
# sockets/logs and trip list/rm tests.
rm -rf "$SESSDIR" 2>/dev/null || true

cleanup() {
    rm -rf "$TESTDIR"
    rm -rf "$SESSDIR" 2>/dev/null || true
    # Sibling dir created by the ssh2incus-atch symlink test (section 7).
    rm -rf "/tmp/.ssh2incus-atch-$(id -u)" 2>/dev/null || true
}
trap cleanup EXIT
...
run env ATCH_SESSION="$SESSDIR/s-clear-cur" "$ATCH" clear
...
run env ATCH_SESSION="$SESSDIR/s-clear-cur:$SESSDIR/s-clear-inner" \
        "$ATCH" clear
...
run env ATCH_SESSION="$SESSDIR/mywork" "$ATCH" current
...
run env ATCH_SESSION="$SESSDIR/outer:$SESSDIR/inner" \
        "$ATCH" current
...
mkdir -p "$TESTDIR" "$TESTDIR/bin"
ATCH_ABS=$(cd "$(dirname "$ATCH")" && pwd)/$(basename "$ATCH")
ln -s "$ATCH_ABS" "$DASH_ATCH" 2>/dev/null || cp "$ATCH_ABS" "$DASH_ATCH"
...
sleep 0.1
run grep -q "atch-push-marker" "$SESSDIR/s-push.log"
...
sleep 0.1
run grep -q "hello-log-test" "$SESSDIR/log-test.log"
...
if [ -e "$SESSDIR/C-global.log" ]; then
...
sleep 0.1
printf "stale\n" > "$SESSDIR/s-rma-stale.log"
printf "exited\n" > "$SESSDIR/s-rma-exited.log"
