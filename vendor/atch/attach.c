...
void write_packet_or_fail(int fd, const struct packet *pkt)
{
	...
	size_t remaining = sizeof(struct packet);

	/* BrainyBlaze fork: retry partial writes (mirror of write_buf_or_fail).
	** With the extended payload (BUFSIZE), packet size is ~4KB and partial
	** writes on a unix-domain socket under load are no longer hypothetical.
	*/
	while (remaining > 0) {
...
int replay_session_log(int saved_errno)
{
	...
	logfd = atch_storage_open_path_file(log_path, O_RDONLY, 0600);
	if (logfd < 0)
		return 0;
