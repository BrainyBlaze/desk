...
// using a durable receipt set keyed by eventId, plus a durable cursor.
//
...
import { type AcceptedEvent } from './model.js';
 ...
 *  - `hasReceipt` — is this eventId's effect already applied? (durable set)
 *  - `applyAndReceipt` — ATOMIC (outbox): run `effect` and record `eventId`'s
 ...
 * committed). An entry whose eventId already has a receipt (a replay, or an
 ...
 * horizon (a receipt is only needed while its eventId can still be replayed).
 */
export class InMemoryConsumerStore implements ConsumerStore {
  ...
  prune(keep: (eventId: string) => boolean): void {
