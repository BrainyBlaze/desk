...
export class SessionRuntime {
  ...
  subscribe(
  ...
  ): { channelId: number; commanded?: CommandedGeometry } {
    ...
    // A new subscriber starts visible: the client subscribes only revealed
    // surfaces — hide is UNSUBSCRIBE, reveal is a fresh SUBSCRIBE
    // (binaryTerminalBrokerClient.setVisibility), never a hidden mount.
    ...
    // post-command revision and the snapshot text is serialized at the geometry
    // the surface will actually render.
    this.d.sendBrowser(channelId, {
   ...
   */
  onBrowserResize(channelId: number, rows: number, cols: number): CommandedGeometry | undefined {
