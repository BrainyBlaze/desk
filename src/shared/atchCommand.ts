// The atch child command for a session — ONE implementation shared by the
...
/**
 * Run the session's command in its cwd, exactly as `tmux new-session -c cwd
 ...
 */
export function atchCommandFor(spec: { command?: string; cwd?: string }): string[] {
