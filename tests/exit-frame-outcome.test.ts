...
describe('the browser EXIT frame tells the truth about an unprovable ending (desk#70)', () => {
  ...
  it('renders a signalled ending as signalled with the signal number, not as code 143', () => {
    ...
    const observed = mgr.observeMoorEvent('sess', generation, {
      type: 'exit',
      outcome: { kind: 'signalled', signal: 15, method: 'none' },
      outputEnd: 0n,
      ts: 1.028
    });
    expect(observed).toMatchObject({ ok: true, authority: { kind: 'applied' } });
    ...
    const exitFrames = daemonFrames.filter((frame) => frame.type === BpFrameType.EXIT);
    expect(exitFrames).toHaveLength(1);
    expect(exitFrames[0]).toMatchObject({ outcome: { kind: 'signalled', signal: 15, method: 'none' } });
    expect(exits).toEqual([{ kind: 'signalled', signal: 15, method: 'none' }]);
...
describe('describeSessionExit — the terminal line for each moor ending', () => {
  it('states each ending in its own words, and names the method only when the holder asked', () => {
    expect(describeSessionExit({ kind: 'exited', code: 0, method: 'none' })).toBe('[session exited 0]');
    expect(describeSessionExit({ kind: 'exited', code: 143, method: 'none' })).toBe('[session exited 143]');
    // `method` is the independent "did the holder ask it to end" axis; shown only when it is not `none`.
    expect(describeSessionExit({ kind: 'exited', code: 0, method: 'graceful' })).toBe('[session exited 0 (graceful)]');
    expect(describeSessionExit({ kind: 'exited', code: 1, method: 'forced' })).toBe('[session exited 1 (forced)]');
    expect(describeSessionExit({ kind: 'signalled', signal: 9, method: 'none' })).toBe('[session signalled 9 (SIGKILL)]');
    expect(describeSessionExit({ kind: 'signalled', signal: 9, method: 'forced' })).toBe('[session signalled 9 (SIGKILL), forced]');
  ...
  it('names only signals whose number is the same on every POSIX platform, else the number alone', () => {
    ...
    expect(describeSessionExit({ kind: 'signalled', signal: 1, method: 'none' })).toBe('[session signalled 1 (SIGHUP)]');
    expect(describeSessionExit({ kind: 'signalled', signal: 2, method: 'none' })).toBe('[session signalled 2 (SIGINT)]');
    expect(describeSessionExit({ kind: 'signalled', signal: 10, method: 'none' })).toBe('[session signalled 10]');
    expect(describeSessionExit({ kind: 'signalled', signal: 64, method: 'none' })).toBe('[session signalled 64]');
  });
});
