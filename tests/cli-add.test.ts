...
async function runAdd(args: string[]): Promise<{ code: number; stderr: string }> {
  const errors: string[] = [];
  const errSpy = vi.spyOn(console, 'error').mockImplementation((line = '') => errors.push(String(line)));
  const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
  try {
    return { code: await main(['add', ...args]), stderr: errors.join('\n') };
  } finally {
    errSpy.mockRestore();
    logSpy.mockRestore();
  }
...
describe('desk add --agent validation', () => {
  ...
  it('rejects an unsupported agent and does not write the config (no brick)', async () => {
    const res = await runAdd(['--file', manifest, '--group', 'g', '--name', 'n', '--cwd', '/tmp', '--agent', 'gemini', '--resume', 'x']);
